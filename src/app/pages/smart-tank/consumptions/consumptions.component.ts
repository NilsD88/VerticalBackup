...
export class ConsumptionsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activeRoute.params.subscribe(async (params) => {
      if (params.id) {
        this.subs.add(
          this.smartTankAssetService.getAssetDetailById(params.id).subscribe(
            (asset) => {
              this.asset = asset;
  ...
  private getChartData(request: Observable<any>) {
    return request.pipe(
      ...
      switchMap(filter => {
        ...
        this.changeDetectorRef.detectChanges();
        return this.smartTankAssetService.getAssetDataById(this.asset.id, filter.interval, filter.from, filter.to).pipe(
          catchError(() => {

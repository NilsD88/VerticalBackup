...
export class PlaceWizardComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public stairway2HealthLocationService: Stairway2HealthLocationService,
  ...
  private async resetLocation(parentId) {
    this.location = {
      ...
      geolocation: null,
      customFields: [],
      module: 'PEOPLE_COUNTING_STAIRWAY_2_HEALTH'
    };
  ...
  public submitAndContinue() {
    ...
    if (this.editMode || !isNullOrUndefined(this.location.id)) {
      ...
      this.subs.add(
        this.stairway2HealthLocationService.createLocation(this.location).subscribe(

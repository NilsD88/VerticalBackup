...
export class TrailWizardComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public walkingTrailsLocationService: WalkingTrailsLocationService,
  ...
  async ngOnInit() {
    ...
    if (!isNullOrUndefined(locationId) && locationId !== 'new') {
      try {
        this.location = await this.walkingTrailsLocationService.getLocationById(locationId).toPromise();
        this.assetsRequest$.next();
        this.editMode = true;
        this.originalLocation = cloneDeep(this.location);
        this.subs.add(
          this.walkingTrailsLocationService.getImageCollectionById(locationId).subscribe(location => {
  ...
  public submitAndContinue() {
    ...
    if (this.editMode || !isNullOrUndefined(this.location.id)) {
      ...
      this.subs.add(
        this.walkingTrailsLocationService.createLocation(this.location).subscribe(
  ...
  public cancelWizard() {
    this.router.navigateByUrl('/private/walking-trails/dashboard');
  }

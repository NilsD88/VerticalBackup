...
prebuild.on('close', (code) => {
  ...
  console.log(`node --max-old-space-size=8192 node_modules/@angular/cli/bin/ng build ${config}`);
  const build = spawn(`node --max-old-space-size=8192 node_modules/@angular/cli/bin/ng build ${config}`, {
    shell: true
  });
